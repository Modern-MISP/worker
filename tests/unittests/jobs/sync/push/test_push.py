...
class TestPush(TestCase):
    ...
    def test_push_add_event_full(self):
        ...
        self.assertTrue(test_push_worker.misp_api.save_event_dic(new_event))
        ...
        server: MispServer = test_push_worker.misp_api.get_server(1)

        # if event wasn't pushed to remote-server it throws Exception
        test_push_worker.misp_api.get_event_by_uuid(new_event["uuid"], server)
    ...
    def test_push_edit_event_full(self):
        ...
        self.assertTrue(test_push_worker.misp_api.update_event_dic(new_event))

        push_job(user_data, push_data)

        # tests if event was updated on remote-server
        remote_event: MispEvent = test_push_worker.misp_api.get_event_by_uuid(new_event["uuid"], server)
