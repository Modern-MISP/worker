...
def test_enrich_attribute_job(attribute_with_normal_tag, view_only_user):
    attribute_id: int = attribute_with_normal_tag.id
    attribute_type: str = attribute_with_normal_tag.type

    plugin_mock: Mock = _generate_mock_plugin(PluginIO(INPUT=[attribute_type], OUTPUT=[attribute_type]))
    ...
    job_data: EnrichAttributeData = EnrichAttributeData(
        attribute_id=attribute_id, enrichment_plugins=[plugin_mock_name]
    )

    pseudo_result_attribute: AddAttributeBody = AddAttributeBody(type=attribute_type)
...
#         self._EXAMPLE_ATTRIBUTE, plugins_to_execute
#     )
#
