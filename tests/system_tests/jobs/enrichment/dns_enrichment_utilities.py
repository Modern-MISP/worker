...
class DNSEnrichmentUtilities:
    ...
    def _create_event() -> int:
        ...
        event_response: Response = requests.post(f"{old_misp_url}/events/add", headers=old_misp_headers,
                                                 json=event_body)
    ...
    def _create_domain_attribute(cls, event_id: int, domain: str) -> int:
        ...
        attribute_response: Response = (
            requests.post(f"{old_misp_url}/attributes/add/{event_id}", headers=old_misp_headers,
                          json=attribute_body))
