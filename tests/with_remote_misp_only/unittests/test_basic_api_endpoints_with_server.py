...
@pytest.mark.asyncio
async def test_get_event_from_server(init_api_config, misp_api, remote_misp, remote_event):
...
async def test_get_server(init_api_config, misp_api, server):
    get_server_result: Server = await get_server(server.id)
    assert server.name == get_server_result.name
    assert server.url == get_server_result.url


@pytest.mark.asyncio
async def test_get_custom_clusters_from_server(init_api_config, misp_api, remote_misp, remote_test_default_galaxy):
    server: Server = await get_server(remote_misp.id)
...
@pytest.mark.asyncio
async def test_get_galaxy_cluster_from_server(init_api_config, misp_api, remote_misp, remote_test_default_galaxy):
    server: Server = await get_server(remote_misp.id)
    cluster = await misp_api.get_galaxy_cluster(remote_test_default_galaxy["galaxy_cluster"].id, server)
    assert cluster.uuid == remote_test_default_galaxy["galaxy_cluster"].uuid


@pytest.mark.asyncio
async def test_get_minimal_events_from_server(init_api_config, misp_api, remote_misp, remote_event, remote_event2):
    server: Server = await get_server(remote_misp.id)
