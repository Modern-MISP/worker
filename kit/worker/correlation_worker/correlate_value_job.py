...
class CorrelateValueJob(Worker):
    ...
    def run(self, value: str) -> CorrelateValueResponse:
        ...
        attributes: List[EventAttribute] = self._misp_sql.fetch_attribute_correlations(value)
        # TODO fragen: if len(attributes) > CorrelationWorker.get_trehsold
        self._misp_sql.add_over_correlating_value(value)
        self._misp_sql.add_correlation_value(value)
